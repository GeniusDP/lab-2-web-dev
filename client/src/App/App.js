...
function App() {
    ...
    function onSubmitHandler(event) {
        ...
        fetch('/send_info/',
            ...
            if(body.permission === 'no')alert('You cannot write a new letter more the 1 time per 30 sec. Try later.');
                else {
                    if (body.sent === true) alert('Your letter was sent.');
                    else alert('Sending of your letter was aborted (look through your password and e-mail to fix the problem)' +
      ...
      <form onSubmit={onSubmitHandler} className="App">
          ...
          <button
              ...
              onClick={
                  (event)=>{
                      event.preventDefault();
                      if(yourName.length>0 && yourSurname.length>0 && text.length>0) {
